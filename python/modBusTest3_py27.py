...
while (count < nSets):
  ...
  dSets[count] = rr.registers[0] + (rr.registers[1] << 16)
  if (count > 0):
      # Catch duplicates and don't save them.
      if (dSets[count] == dSets[count-1]):
  	  continue
  ptCount = rr.registers[2]

  #print 'Set #: ' + str(count) + '; Pt. Count: ' + str(ptCount)
  ...
  for dSet in range(ptCount):
      ...
      times[dIdx] = rr.registers[3] + (rr.registers[4] << 16) + (rr.registers[5] << 32) + (rr.registers[6] << 48)
      dIdx += 1
...
for i in range(dIdx):
    print str(i) + ', ' + str(times[i]) + ',' + str(displacements[i])

print 'Points collected: ' + str(dIdx)
print "Done."
