...
define(function(require, exports, module) {
    ...
    const createWS = (callback, options) => {
      if (navigator.onLine) {
        if ("WebSocket" in window) {
          ...
          var myWS = new ReconnectingWebSocket("ws://192.168.168.41:3405",
                                               null,
    ...
    const usesPlugin = () => { return communicationChannel === "Plugin"; };
    const usesWebSocket = () => { return communicationChannel === "WebSocket"; };
    ...
    const sendMessage = (message) => {
      if (usesWebSocket()) {
        e4PtSocket.send(JSON.stringify({
          text: message,
          type: "message",
          id: clientID,
          date: Date.now()
